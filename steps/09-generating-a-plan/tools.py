...
4. Writing Style: Write in cohesive prose rather than fragmented notes. Organize the report with descriptive section headers (e.g., "## San Francisco Climate"), where each header is 5-7 words long. Prefer depth and synthesis over terse summaries. Prefer longer, detailed paragraphs where possible. Ensure the final report is comprehensive and substantial, aiming for a length of at least 1500 to 2000 words to provide a deep dive into the topic.
...
9. Final Output: Save the report to a markdown file using write_file or edit_file. Do not dump the whole report into the chat response. Give the user a concise update on what you researched and what sources you used. As the final step before finishing the task, ensure the report starts with a `#` header containing a funny quip/title that summarizes the entire thing, followed by a 3-5 sentence executive summary (without an explicit "Executive Summary" header) at the top, and a short conclusion section at the end that wraps up everything.
